...
class CreateGuacamoleDevice(APIView):
    ...
    def post(self, request, pk, *args, **kwargs):
        ...
        if auth_response.status_code != 200:
            return Response({"error": "Failed to authenticate with Guacamole"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        if list_response.status_code != 200:
            return Response({"error": "Failed to retrieve connections from Guacamole"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        if existing_device:
            ...
            if detail_response.status_code != 200:
                return Response({"error": "Failed to retrieve connection details from Guacamole"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            if parameter_response.status_code != 200:
                return Response({"error": "Failed to retrieve connection parameters from Guacamole"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)

            existing_parameters = parameter_response.json()

            # Compare the existing device's parameters with the new payload
            if existing_details['protocol'] != payload['protocol'] or \
               existing_parameters != payload['parameters']:
                # Update the device
                ...
                if update_response.status_code != 204:
                    return Response({"error": "Failed to update the device in Guacamole"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
                return Response({"message": "Device updated successfully in Guacamole"}, status=status.HTTP_202_ACCEPTED)
            else:
            ...
            if create_response.status_code != 200:
                return Response({"error": "Failed to create the device in Guacamole"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            return Response({"message": "Device created successfully in Guacamole"}, status=status.HTTP_202_ACCEPTED)
...
class GetGuacamoleClient(APIView):
    ...
    def get(self, request, pk, *args, **kwargs):
        ...
        return Response({"clientid": clientid, "url": f"{guac_frontend_url}/guacamole/#/client/{clientid}?token={token}"}, status=status.HTTP_200_OK)
