...
- Nautobot Configuration for:
    ...
    - guac_data_source - The [Guacamole datasource](https://github.com/apache/guacamole-client/blob/fff7ba41225586501869194b46db8a36ea910cb1/guacamole-docker/bin/initdb.sh#L57-L65) that is being used, one of postgresql, mysql, or sqlserver.
...
> Note: The SSH connection "fakes" a `Login as:` prompt, so if you see that prompt and then Guacamole reports it is needs to reconnect, it likely never connected. This makes sense since you send your username when you SSH, and this is Guacamole's way to provide the user to define the username.

## Current Shortcomings

The Docker configuration configures guacamole-preparation to get around a chicken and egg issue in which you need an sql file to be generated, which is then required by the database, which is then required by the Guacamole service. There is likely a way around this, but not found at the time of this writing.
